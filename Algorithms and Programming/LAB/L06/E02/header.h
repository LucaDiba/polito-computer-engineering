...
typedef Atleta Item;
typedef struct{
    Atleta **byCodice, **byCognome, **byDataNascita, **byCategoria, **byAcquisizione;
} AtletaOrder;
typedef struct{
...
typedef struct{
    int byCodice, byCognome, byDataNascita, byCategoria;
} Ordered;
typedef enum{
    less, equal, greater
} CheckType;
...
n_Info leggiFile(Atleta **passed_atleti, Categoria **passed_categorie, AtletaOrder *orderby);
void stampaAtleta(Atleta *atleta, Categoria **categorie, FILE *fp);
void stampaElenco(Atleta **, int n_atleti, Categoria **categorie);
...
n_Info leggiFile(Atleta **passed_atleti, Categoria **passed_categorie, AtletaOrder *orderby){
    ...
    orderby->byAcquisizione = malloc(n_info.n_atleti * sizeof(Atleta *));
    orderby->byCategoria = malloc(n_info.n_atleti * sizeof(Atleta *));
    orderby->byCognome = malloc(n_info.n_atleti * sizeof(Atleta *));
    orderby->byCodice = malloc(n_info.n_atleti * sizeof(Atleta *));
    orderby->byDataNascita = malloc(n_info.n_atleti * sizeof(Atleta *));
    ...
    for(i=0; i<n_info.n_atleti; i++){
        ...
        orderby->byAcquisizione[i] = &(atleti[i]);
        orderby->byCategoria[i] = &(atleti[i]);
        orderby->byCognome[i] = &(atleti[i]);
        orderby->byCodice[i] = &(atleti[i]);
        orderby->byDataNascita[i] = &(atleti[i]);
    }
...
void stampaCategorie(Atleta **atleti, Categoria *categorie, n_Info n_info){
